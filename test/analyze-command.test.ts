...
describe('runAnalyzeCommand', () => {
  ...
  it('writes a report with results and summary to --out', async () => {
    ...
    expect(report.summary).toMatchObject({ total: 1, ok: 1, failed: 0, model: 'qwen3.5:4b' });
  ...
  it('fails preflight with exit code 3 and remediation on stderr', async () => {
    ...
    expect(code).toBe(3);
    expect(cap.err()).toContain('ollama pull qwen3.5:4b');
  });
  ...
  it('emits a single AnalysisResult envelope on stdout when preflight fails with --json', async () => {
    ...
    expect(payload.error?.message).toContain('ollama pull qwen3.5:4b');
